...
$("#canvas-wrap")[0].ondrop = function(e) {
	if (dragEle) {
		...
		img.onload = function() {
			...
			$('#move').trigger('click');
			pomelo.send('connector.roomHandler.draw',
				{
...
$('#chatSubmit').click(function () {
	...
	if (!value) return;
	pomelo.notify('connector.roomHandler.chat',
		{
...
$('#p-end-course .pop-btn-submit').click(function () {
	...
	pomelo.notify('connector.roomHandler.leave',
		{
			roomID: user.roomID,
			username:user.username,
			role :user.role
...
pomelo.on('room.enter_push', function (data) {
	...
	log(data.role + data.username + '已加入房间');
})
...
pomelo.on('room.leave_push', function (data) {
	...
	log(data.role + data.username + '已离开房间');
